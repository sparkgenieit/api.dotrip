/*
  ...
  - A unique constraint covering the columns `[assignedVehicleId]` on the table `Driver` will be added. If there are existing duplicate values, this will fail.
...
-- DropIndex
DROP INDEX `Driver_vehicleId_key` ON `driver`;
...
ALTER TABLE `driver` DROP COLUMN `createdAt`,
    ...
    ADD COLUMN `assignedVehicleId` INTEGER NULL,
    ADD COLUMN `email` VARCHAR(191) NOT NULL,
    ADD COLUMN `fullName` VARCHAR(191) NOT NULL,
    ADD COLUMN `isAvailable` BOOLEAN NOT NULL DEFAULT true,
    ADD COLUMN `isPartTime` BOOLEAN NOT NULL DEFAULT false,
    ADD COLUMN `licenseExpiry` DATETIME(3) NOT NULL,
