...
-- DropIndex
DROP INDEX `Booking_vendorId_fkey` ON `booking`;
...
-- DropIndex
DROP INDEX `Quote_bookingId_fkey` ON `quote`;

-- DropIndex
DROP INDEX `Quote_vendorId_fkey` ON `quote`;

-- DropIndex
DROP INDEX `Trip_bookingId_fkey` ON `trip`;
...
-- DropIndex
DROP INDEX `Vehicle_driverOwnerId_fkey` ON `vehicle`;
...
-- AlterTable
ALTER TABLE `vehicle` ADD COLUMN `priceId` INTEGER NOT NULL DEFAULT 10;

-- AlterTable
ALTER TABLE `vehicletype` ADD COLUMN `seatingCapacity` INTEGER NOT NULL DEFAULT 4;
...
-- AddForeignKey
ALTER TABLE `Vehicle` ADD CONSTRAINT `Vehicle_priceId_fkey` FOREIGN KEY (`priceId`) REFERENCES `Price`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
